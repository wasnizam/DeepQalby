      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="space-y-6">
          {clients.map((client, idx) => (
            <div key={idx} className="flex items-start gap-4 p-6 rounded-2xl bg-slate-50 border border-slate-100/50">
              <div className="mt-1 shrink-0 w-5 h-5 rounded-full bg-slate-900 text-white flex items-center justify-center">
                <Check size={12} strokeWidth={3} />
              </div>
              <span className="text-lg text-slate-800">{client}</span>
            </div>
          ))}
        </div>
      </div>
